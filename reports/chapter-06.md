...
第4章で環境を作り、第5章で自然言語だけで分析を一周させました。ここで一度立ち止まります。「**動く**」ことと「**安全に運用できる**」ことは別の問題だからです。

研究現場では、動くだけの分析は次のような事故と隣り合わせです。
...
Copilot CLI には、**どのツールを使わせるか**をコマンドラインオプションで細かく指定する仕組みがあります。研究業務では、これを積極的に使います。
...
対話中に `/permissions` と入力すると、現在有効な許可・拒否ルールを確認できます。**分析セッションを始める前に必ず一度見る**習慣をつけると、事故が減ります。
...
> [!TIP]
> **「このデータを AI に渡してよいか？」**を毎回考えるのは疲れます。ラベル運用（`public/` / `internal/` / `confidential/` などのディレクトリ分け）を先に決めてしまうと、判断が自動化できます。
...
- **動く≠安全**。**動かす前に**、最小権限・実行前承認・出口管理の3原則で運用を引き締める
