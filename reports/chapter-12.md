...
def check(record: dict) -> list[str]:
    errs = []
    # (A) ネスト形式：peaks 配列（第9章）
    for peak in record.get("peaks", []):
        for key in RANGES:
            if key in peak:
                _check_value(key, peak[key], errs)
    # (B) フラット形式：トップレベルにキー・配列（第10・11章の派生量）
...
if __name__ == "__main__":
    ...
    for k, vs in data.items():
        ...
        if status == "ok":
            ...
            inconclusive = True
    sys.exit(2 if inconclusive else 0)      # exit=2 で "判定不能" を明示
```

> [!IMPORTANT]
> **`no outliers` と `判定不能` を混同しない**：終了コード `0` は「判定できて外れ値なし」、`2` は「判定できていない（人間の確認が必要）」を意味します。§12.2 の物理的妥当性チェックと同じ設計思想です。
...
第9章の実機検証で得た `[519.65, 1331.96, 2900.88]` cm⁻¹ に対する「独立基準」の例：
- シリコン標準：520 cm⁻¹ の公称ピーク（校正済み装置の再現性±1 cm⁻¹）
- ダイヤモンド一次ラマン：1332 cm⁻¹ の代表値（校正済み装置の再現性±2 cm⁻¹）
- 2900.88 cm⁻¹：**独立基準未設定**（本書の合否判定には使わない。もし基準が必要なら、C-H 伸縮振動域の代表値を装置校正済み条件で別途取得すること）
- **一致基準の例**：`|detected - reference| ≤ 2 cm⁻¹ を "一致"、5 cm⁻¹ 超は "再検証"`

> [!IMPORTANT]
> **これは "帰属" ではありません**：本章は「基準値と数値が近いこと」を確認する作業までです。「よって試料はシリコンである」という**同定は人間の判断**です（第9・10章の禁止事項と同じ）。
...
- [ ] **②入力条件**：データ契約 7 要素を満たす。**マルチモーダル Skill の場合はさらに `join_key_type` / `join_key_value` / `units` / `quantities` が入力契約に含まれる**（第11章 canonical shape）
- [ ] **③出力形式**：`references/output-schema.json` があり、JSON Schema で valid。**マルチモーダルは `missing_modalities` と構造化 `provenance`（`skill_version` / `modality_versions` / `run_datetime_utc`）を必須項目に含む**
- [ ] **④成功条件**：数値で書ける（曖昧な "正しく動く" は NG）
- [ ] **⑤禁止事項**：**物質同定・帰属を含まない**（第9〜11章共通）
- [ ] **⑥再現性条件**：`skill_version`, `package_versions`, `random_seed` を記録。**浮動小数点処理を含む場合は "同一と見なす許容差" を事前に明記**（§12.4 L1）
...
- [ ] **⑫再現性 L1**：同一環境・同一乱数種で 2 回実行して数値一致
...
- [脚注1] Median Absolute Deviation の概念と補正係数 0.6745 の根拠：Rousseeuw, P. J., & Croux, C. (1993) "Alternatives to the Median Absolute Deviation", Journal of the American Statistical Association 88(424):1273-1283. https://doi.org/10.1080/01621459.1993.10476408
