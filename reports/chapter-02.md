...
| 観察・イメージング | 光学顕微鏡、走査型顕微鏡、透過電子顕微鏡、膜厚・粒度測定 |
...
```text
前提: 何を知りたいか（問い） / 成功条件は何か

[1] 取得・入手        [2] 整形・品質確認      [3] 可視化・観察
   データを集める  →   読み込み・単位確認  →   まず眺めて把握
   目的/試料ID/          欠損・外れ値処理
   測定条件/校正確認

[4] 特徴抽出・解析     [5] 解釈・報告
   ピーク/相/粒径等  →   考察・文献照合・人間による妥当性判断
   数値化・定量化        不確かさ/再現性/既知結果との整合、再現可能な記録
```
...
|---|---|---|---|
| **スペクトル型** | 横軸=波長/周波数/質量など、縦軸=強度・応答量の1次元信号 | 各種分光（状態分析）、磁気共鳴、質量分析、EIS | ベースライン補正、ピーク検出、面積・比の算出 |
| **クロマトグラム・時系列型** | 横軸=時間/温度/電位/深さ/サイクルなど、それに対する強度・物理量の変化（1次元プロファイル） | クロマトグラフ、熱分析、プロセス条件ログ | ピーク分離、保持時間、変化点・トレンド検出 |
| **画像・顕微鏡型** | 2次元（またはスタック）の画像データ | 光学顕微鏡、走査型顕微鏡、透過電子顕微鏡 | セグメンテーション、粒径・形状計測、計数 |
| **回折・散乱パターン型** | 角度/散乱ベクトルに対する強度、ピークやリング | 回折・散乱（XRD等） | ピーク指数付け、相同定、格子定数算出 |
...
> [!IMPORTANT]
> **分類は「装置名」ではなく「分析したいデータの構造」で行う**のが鉄則です。同じ装置カテゴリでも、出力によって型が変わります。迷ったら次の判定フローを使ってください。

```text
データ型を選ぶときは、装置名ではなく「分析したい単位」を見る。

1. 横軸が波長・波数・質量・周波数などで、ピーク/応答を見る → スペクトル型
2. 横軸が時間・温度・電位・深さ・サイクル・プロセス進行で、変化やプロファイルを見る → クロマトグラム・時系列型
3. ピクセル・粒子・領域を扱う                          → 画像・顕微鏡型
4. 角度・q値・結晶相・リング/スポットを見る            → 回折・散乱パターン型
5. 行が試料/条件、列が特徴量/測定値                    → 表形式・プロセス条件型
6. 複数の型を対応づけて解釈する                        → マルチモーダル統合型
```
...
| 装置・研究活動 | 主に対応する型 | 注意点 |
|---|---|---|
| 電気化学 | 時系列型（充放電・CV: 電位/時間に対する応答）／スペクトル型（EIS: 周波数応答、Bode/Nyquist）／表形式型（サイクル寿命表） | 測定モードで型が変わる |
| 表面分析 | スペクトル型（XPS/AES）／クロマトグラム・時系列型（SIMS深さ方向プロファイル）／画像型（表面マッピング） | 単一の型に固定しない |
...
```text
スペクトル型Skillの骨格（例）
[1] 取得   : ファイル読込（CSV/装置固有形式）、軸と単位の確認
[2] 整形   : ノイズ除去、ベースライン補正、正規化
[3] 可視化 : スペクトル全体をプロット、対象領域を拡大
[4] 特徴   : ピーク検出 → 位置・強度・面積の算出
[5] 解釈   : ピーク帰属、試料間比較、文献照合（第10章）
```

この骨格は、**赤外分光でもラマンでも質量分析でも、大枠は共通**です。違うのは「どの前処理を選ぶか」「ピークをどう帰属するか」といった**中身の調整**だけです。sai
...
```text
                   ┌─────────────── AIエージェント（Copilot CLI）──────────────┐
                   │  自然言語の指示を受け、ステップを実行し、結果を人間に返す     │
                   └───────────────────────────┬─────────────────────────────┘
                                                │ MCP でツールに接続
   [1]取得 ──[2]整形 ──[3]可視化 ──[4]特徴抽出 ──[5]解釈・報告
      │         │         │            │              │
   Jupyter MCP（ノートブックで実行）… 実行の共通基盤                │
                                         [5]解釈・報告
                                          ├ arXiv / Paper Search MCP（文献照合）
                                          ├ ToolUniverse MCP（専門ツール呼び出し）
                                          └ 人間が妥当性を判断（最終判断）
   ────── これらの「分析の型」を Skill としてまとめて再利用 ──────
```
