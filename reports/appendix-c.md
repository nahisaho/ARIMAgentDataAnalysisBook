...
- [ ] JupyterLab / MCP サーバは **今も起動しているか** をブラウザ / `copilot mcp list` で確認した
...
- `which copilot` が空 → `npm install -g @github/copilot-cli` を再実行（第4章 §4.5）
...
- 未 install → `pip install uv`（第4章 §4.4）または公式スタンドアロンインストーラ
- install 後に反映されない → **ターミナル再起動** or `hash -r`
- 会社 PC で pip が制限されている → `pipx install uv` または公式スタンドアロン
...
```bash
# 本書標準環境の版に揃える（第4章 §4.3）
pip install "jupyter-collaboration>=3.0" "pycrdt>=0.10"
...
- トークンが毎回変わって煩雑なら、`jupyter lab --NotebookApp.token=my-fixed-token` で固定（**個人開発機のみ**、共有サーバでは危険）
- 一度 `copilot mcp remove jupyter` してから `copilot mcp add jupyter --env ...` で登録し直す
...
- ネットワーク（企業プロキシ・VPN）でブロックされていないか `curl -I https://arxiv.org/` で疎通確認
...
```bash
# Copilot CLI を起動したシェルで値が見えるか
echo "$JUPYTER_TOKEN"
echo "$OPENAI_API_KEY"

# copilot mcp add で登録した場合の中身
copilot mcp get jupyter --show-secrets    # マスク解除
```
...
2. **意図的故障ドリル**：`copilot mcp remove jupyter` で Jupyter MCP を外し、Skill を実行してみる。どの症状表（C.3.2 / C.3.3）にヒットするかを追い、`add` で戻して回復するまでを 5 分以内で行う。
