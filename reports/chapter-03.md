...
2. **道具を自分で選んで使う** —— ファイル読込・可視化・計算などを状況に応じて実行する
3. **文脈を保って反復する** —— 結果を見て次の手を考え、対話的に分析を深める

ただし、AI Agent 単体では「言葉を理解して応答する」ことが中心です。**実際にノートブックを動かしたり、文献を検索したりするには、Agent が外部の道具に接続する必要があります**。本書では、その接続の共通規格として MCP を使います。
...
| **MCPホスト／クライアント** | AI Agent 側で、MCPサーバにつないで道具を使う側 | GitHub Copilot CLI |

> [!NOTE]
> 厳密には、GitHub Copilot CLI のようなAIアプリが **MCPホスト** であり、その内部に各MCPサーバへ接続する **MCPクライアント** が作られます。本書では初学者向けに、これらをまとめて「Agent側」と呼ぶことがあります。実装レベルの詳細は付録Bで扱います。
...
> [!TIP]
> 「Skill＝手順書」「AI Agent＝その手順書に従って動く作業者」という関係です。同じ手順書（Skill）を、別の作業者（別のAI Agent環境）でも、別のデータでも使い回せる——だからこそ、まず1つの型で丁寧に Skill を作ることに価値があります（第9章）。
...
flowchart TB
    ...
    subgraph MCPs["MCP 経由で接続する外部ツール"]
        ...
        Lit["arXiv / Paper Search MCP<br/>文献検索"]
...
2. **AI Agent が中心のハブ** —— 人間・Skill・道具のすべてが AI Agent を介してつながります。
...
- [脚注1] 大規模言語モデルを用いたAIエージェント／コーディングエージェントの例: GitHub Copilot CLI (https://docs.github.com/copilot)、Claude (https://www.anthropic.com)、OpenAI (https://platform.openai.com/docs)。
