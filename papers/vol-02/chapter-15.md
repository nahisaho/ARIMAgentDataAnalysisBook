...
```
Skill を組織展開したい
├─ 手法は安定しているか？（Major 3か月不変）
│   ├─ No → パターン A（Skill 共有）
│   └─ Yes ↓
├─ 監査ログを一元管理する必要があるか？
│   ├─ No → パターン A で十分。パターン C も検討
│   └─ Yes ↓
├─ 誤用が起きたら業務影響が大きいか？（品質判定・出荷判定など）
│   ├─ No → パターン A + 定期監査でも可
│   └─ Yes → パターン B（専用 MCP 化）
└─ 実装リソース（Python 開発 + テスト）を確保できるか？
    ├─ No → パターン A に留めて再検討
    └─ Yes → パターン B 実装へ
```
...
| 優先度 | Skill | 理由 |
|---|---|---|
| 1 | **Ch6 Pipeline + CV 評価** | 入出力が明確（X, y, cv_scheme → scores）。API 設計しやすい |
| 2 | **Ch10 校正曲線推定** | 入出力が明確（X, y → posterior + calibration params）。実務利用頻度高 |
| 3 | **Ch12 診断チェック** | Ch14 §14.6 の `check_diagnostics()` そのもの。ライブラリ化容易 |
| 4 | Ch11 階層モデル | 構造が多様で API 設計に工夫が要る。プロジェクト個別性が高い |
| 5 | Ch13 Capstone 統合 | provenance チェイン管理が複雑。MCP 化より orchestration 側 |
...
Ch4-14 で拡張してきた provenance を、**組織監査に耐える形**にまとめます。

### 15.5.1 誰が・いつ・どのバージョンで動かしたか

以下 6 項目は監査の**最小共通項**です。
...
| `code_sha256` | Skill ディレクトリの hash | 改変検知 |
...
- `outputs.diagnostics_pass` — Ch12 §12.9 の 14 項目チェック結果
- `outputs.known_issues_snapshot` — 実行時点の `known_issues.yaml` の hash（Ch14 §14.9）
...
> [!IMPORTANT]
> **「実行 → 判断」を切らない**：Ch14 のチェイン切れ（provenance 側）と同様、**実行と業務判断を紐付ける ID**（`decision` セクション）がないと、後日「あの判断は何を根拠にしたか」を追跡できなくなります。
...
| 認証系（品質判定・製造判定） | 業界規定に従う（多くは 5〜10 年以上） | 品質保証部門・監査 |

具体年数は組織のコンプライアンス方針・関連規制（材料分野なら ISO 9001, GLP, GMP 等の適用範囲）に従ってください。本書は**「保存する」こと自体の重要性**を示すに留めます。
...
3. 依存する下流 Skill（Ch13 の Layer 参照）に**警告**（`known_issues.yaml` に `severity: deprecation` を追加）
...
| 決定的な hash 一致 | GPU 非決定性（cuDNN / atomic add） |
| 特徴量エンジニアリングを人間が設計 | 表現学習が自動 → 解釈可能性の再定義 |
| Skill = プロンプト + 少量スクリプト | 学習済み重み（数百 MB〜）の配布・バージョニング |
| PyMC の診断 4 指標 | 学習曲線・early stopping・LR schedule の判断論 |

### 15.8.2 vol-03 想定内容

- **PyTorch / JAX** の Skill 化：訓練・推論・チェックポイント管理
- **Hugging Face MCP**（想定）による事前学習モデル呼び出し
...
vol-02 の Bayesian は「**観測された相関の記述**」に留まります。以下は扱っていません。
...
### Workshop 2：監査ログの最小 6 項目を YAML で書く

§15.5.1 の 6 項目を、あなたの Ch13 Capstone の Layer 1 実行に対して埋めてください。`operator` は仮の ORCID、`code_sha256` は `git rev-parse HEAD` で代用可。
