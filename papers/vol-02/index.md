...
- **対象読者**: ARIMデータポータル会員のデータ分析者（材料・ナノテク研究者。Python/Jupyter 経験あり）。vol-01 未読でも**第0章の最小復習**で読み進められる構造
- **最終ゴール（合格ライン）**:
...
> [!NOTE]
> Stan / cmdstanpy は本文で使用しません（付録B の PyMC↔Stan 対応表として参照用のみ）。深層学習・因果推論は本書のスコープ外です（第1章・第15章で道しるべを示すのみ）。
...
| 章 | タイトル | 状態 |
|---|---|---|
| 第0章 | [vol-01 の最小復習](chapter-00.md) | ✅ 執筆済 |
| 第1章 | [vol-01 の Skill に何が足りないのか](chapter-01.md) | ✅ 執筆済 |
| 第2章 | [ARIM データに現れる統計的課題](chapter-02.md) | ✅ 執筆済 |
| 第3章 | [Scikit-learn と PyMC の全体像・使い分け](chapter-03.md) | ✅ 執筆済 |
| 第4章 | [統計/ML 分析用 Skill の設計原則](chapter-04.md) | ✅ 執筆済 |
| 第5章 | [教師あり学習を Skill 化する](chapter-05.md) | ✅ 執筆済 |
| 第6章 | [教師なし学習を Skill 化する](chapter-06.md) | ✅ 執筆済 |
| 第7章 | [モデル選択・交差検証・データリーク検知](chapter-07.md) | ✅ 執筆済 |
| 第8章 | [解釈可能性とレポート化](chapter-08.md) | ✅ 執筆済 |
| 第9章 | [不確かさ入門：頻度論の限界と Bayesian への橋](chapter-09.md) | ✅ 執筆済 |
| 第10章 | [PyMC 入門と Skill 化](chapter-10.md) | ✅ 執筆済 |
| 第11章 | [階層モデル：反復測定・ロット差・測定誤差](chapter-11.md) | ✅ 執筆済 |
| 第12章 | [MCMC の実務と限界：判断と修正](chapter-12.md) | ✅ 執筆済 |
| 第13章 | [総合ハンズオン（Advanced Capstone）：sklearn → PyMC → 階層モデル](chapter-13.md) | ✅ 執筆済 |
| 第14章 | [統計/ML 特有の失敗パターン](chapter-14.md) | ✅ 執筆済 |
| 第15章 | [組織展開と終章 — Skill 共有・監査ログ・次巻への道しるべ](chapter-15.md) | ✅ 執筆済 |
...
- **vol-01 未読の方**: 第0章 → 第1章 → 第3章 → 第4章 → 第5章（Pillar 1 に進む最短経路）
- **Pillar 1（sklearn Skill）を完成させたい**: 第0〜8章を順に（特に第4章の Skill 設計原則、第7章のリーク検知は飛ばさない）
- **Pillar 2（PyMC Skill）を完成させたい**: 第9〜10章 →（並行して）第4章の Skill 設計 → 第12章の MCMC 診断
- **Advanced Capstone（階層モデル）に挑む**: 第10〜13章。特に第11章（反復測定・ロット差・測定誤差）は付録C の実データ候補と組み合わせて演習
- **自分の現場へ展開したい**: 第14章（失敗パターン）→ 第15章（組織展開）→ 付録A（テンプレート）
