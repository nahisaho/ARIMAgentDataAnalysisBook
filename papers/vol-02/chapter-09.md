...
>
> **推論対象の明示**：パラメータの不確かさ（`parameter`）と、新規試料の予測不確かさ（`prediction` = posterior predictive interval）は**別物**です。材料判断では後者が必要になる場面が多い（第10章、第11章）。
...
**実務のデフォルトは段階 2（弱情報事前分布）**です。段階 1 は「特殊な理論的正当化がある場合のみ」、段階 3 は「事前情報を明示的に検証する場合のみ」使います（第10章）。
...
| リスク | 例 | 予防 |
|---|---|---|
| **事前分布による結論の暴走** | 強い事前分布で観測を無視した結論 | prior predictive check（第10章） |
...
| **確認バイアスの潜入** | 期待する結果を反映した prior | Sensitivity analysis で複数 prior を比較（第10章の感度分析節） |
...
**`sensitivity_alternatives`** を書く理由：後段の「事前分布への感度分析」（第10章）で必要になるため、Skill 設計段階で「代替候補」を凍結しておきます。`data_cutoff_for_prior` は、事後を見てから prior を修正する「後付け汚染」の防止線です。
...
第4章 §4.3 の `task_type` に `bayesian_inference` を追加した理由もここにあります。**成功条件の書き方が根本的に変わる**ため、独立したタスクタイプとして扱います。
...
| **収束診断の省略** | $\hat{R}$ / ESS / divergences / tree depth を見ずに CrI を報告 | Skill 出力に diagnostics_summary を必須化（第10章・第12章） |
...
- 第4章 §4.3：`task_type` に `bayesian_inference` を追加した動機
