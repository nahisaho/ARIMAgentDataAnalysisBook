...
具体的に、どんな場面で vol-01 の Skill だけでは詰まるのかを 4 つ挙げます。第II部以降で、それぞれをどう解くかを扱います。

### 局面 1：多変数を同時に扱いたい（→ 第II部：scikit-learn）

例：熱処理温度・時間・雰囲気ガス組成の 3 変数から、生成物の格子定数と結晶粒径を予測したい。

- **vol-01 での限界**：散布図を並べれば相関は見えるが、**変数間の交互作用**や**非線形性**は目視では追いにくい
- **vol-02 での解**：`RandomForestRegressor` や `GradientBoostingRegressor` などの多変量回帰、`PLSRegression` による多変量校正

### 局面 2：同じ試料を複数回・複数装置で測ったばらつきを分けたい（→ 第IV部：PyMC 階層モデル）

例：3 台の XRD 装置で同じ標準試料を測定した。装置間差 vs 装置内の測定誤差を分離したい。

- **vol-01 での限界**：平均値を取るしかない。**装置間差の大きさ**を数字で言えない
- **vol-02 での解**：階層モデル（`部分プーリング`）による装置効果と観測誤差の分解

### 局面 3：少ないデータで、信頼度付きの予測をしたい（→ 第IV部：PyMC ベイズ推定）

例：10 サンプルしか測れなかった校正曲線から、次の未知試料の濃度を推定したい。

- **vol-01 での限界**：`scipy.optimize.curve_fit` で点推定は出るが、**「濃度は 5.2 ± 何」**が言えない
- **vol-02 での解**：ベイズ校正（事前分布 + データ → 事後分布）による予測分布、95% 予測区間

### 局面 4：モデルが「本当に効いている変数」を見分けたい（→ 第III部：解釈可能性 SHAP・PDP）

例：材料組成 20 変数から硬度を予測するモデルを作った。**どの元素が効いているか**を、共同研究者に説明したい。

- **vol-01 での限界**：`model.coef_` だけ見せても、線形回帰以外では意味が分からない
- **vol-02 での解**：SHAP 値・部分依存プロット（PDP）による特徴量寄与の説明
...
flowchart TD
    ...
    Base --> P1["Pillar 1<br/>scikit-learn Skill<br/>（第II〜III部）"]
    Base --> P2["Pillar 2<br/>PyMC ベイズ Skill<br/>（第IV部）"]
    P1 --> Cap["Advanced Capstone<br/>階層モデル拡張<br/>（第V部）"]
    P2 --> Cap
```

### Pillar 1：scikit-learn Skill（第II部・第III部）
...
### Pillar 2：PyMC ベイズ Skill（第IV部）

- **主用途**：校正曲線のベイズ推定、反応速度の事後分布、装置間差の階層モデル
- **成果物**：`SKILL.md` + `references/` + `tests/` の形で書かれた、`sampler_config` / `posterior_artifact` / `diagnostics_summary` 付きの推定/予測 Skill
- **含まれる規律**：事前分布の物理的正当化、$\hat{R}$ / ESS / divergences の確認、事後予測チェック、`InferenceData` の永続化
- **扱うデータ**：vol-01 のスペクトル型サンプル、ARIM 風の**合成階層データ**（`data/synthetic-hierarchy/`、第2章で入手方法を提示）

### Advanced Capstone：階層モデルの拡張（第V部）

- **主用途**：Pillar 1 と Pillar 2 を組み合わせた、装置間・ロット間・研究室間の階層構造を持つ現実的な材料データへの適用
- **成果物**：Skill の連結（scikit-learn による特徴量エンジニアリング → PyMC による階層モデル推定 → 統合レポート）
- **位置づけ**：**卒業課題**。ここに到達できれば、vol-02 の目的は達成
...
| 道具 | 位置づけ | 選定理由 |
|---|---|---|
| **scikit-learn** | 統計/ML の第一選択 | 材料 ML の標準、`Pipeline` による CV・データリーク防止、SHAP との連携 |
| **PyMC** | ベイズの第一選択 | Python 主導・記述性が高い、`InferenceData` による標準化、階層モデルが自然に書ける |
...
| **SHAP / PDP** | モデル解釈 | scikit-learn モデルの寄与を、共同研究者に説明する標準手段 |
