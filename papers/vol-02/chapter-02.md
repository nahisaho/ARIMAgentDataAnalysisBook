...
>
> **本章で扱わないこと**
> - 統計手法・ML アルゴリズムの実装（第5章以降）
> - vol-01 の失敗事例そのものの詳細（vol-01 第14章）
> - scikit-learn / PyMC の使い分け設計（第4章）

---

## 2.1 vol-01 で作れた Skill と、その先にある壁
...
## 2.2 vol-01 の Skill だけで詰まる 4 つの局面

具体的に、どんな場面で vol-01 の Skill だけでは詰まるのかを 4 つ挙げます。第5章以降で、それぞれをどう解くかを扱います。
...
### 局面 2：同じ試料を複数回・複数装置で測ったばらつきを分けたい（→ 第12章：PyMC 階層モデル）
...
### 局面 4：モデルが「本当に効いている変数」を見分けたい（→ 第9章：解釈可能性 SHAP・PDP）
...
- **扱うデータ**：vol-01 のスペクトル型サンプル、ARIM 風の**合成階層データ**（リポジトリルートの `data/synthetic-hierarchy/`、第3章で入手方法を提示。配置規約は付録A §A.1.1）
...
- **位置づけ**：**卒業課題**。第14章で扱う。第15章は失敗パターン集、第16章は運用と終章
...
## 2.4 vol-02 のバックエンドと道具立て

第5章以降で扱う道具のうち、**「なぜそれを選ぶか」** だけをここで示します。詳細な使い分けは第4章、実装は各章です。

| 道具 | 位置づけ | 選定理由 |
|---|---|---|
| **scikit-learn** | 本書の統計/ML 標準実装 | 材料 ML の広く使われる実装、`Pipeline` による**交差検証（CV：データを分割して汎化性能を評価する手法）**・データリーク防止、SHAP との連携。代替（statsmodels / XGBoost / LightGBM 等）の位置づけは第4章 |
| **PyMC** | 本書のベイズ標準実装 | Python 主導・記述性が高い、`InferenceData` による標準化、階層モデルが自然に書ける。代替（Stan / NumPyro 単独）の位置づけは第4章 |
| **ArviZ** | ベイズの診断・可視化 | PyMC / NumPyro の結果を統一的に扱える |
| **NumPyro（第11章末以降）** | 大規模階層モデル用の高速バックエンド | JAX ベースで、大規模データ・大規模階層での速度が必要な場面のみ導入 |
...
### バックエンド選択の段階導入（第11章末以降）

PyMC のバックエンドは、章によって切り替えます：

- **第11章まで**：デフォルト（PyTensor）を使う。**追加インストールなし**
- **第11章末**：NumPyro / JAX の追加インストール手順を提示（JAX の準備は 5 ページ相当）
- **第12章以降**：階層モデルで NumPyro を実測比較しつつ導入
...
第1章スキップ → **第1〜3章**（本書の枠組み） → **第4〜8章**（scikit-learn） → **第9〜12章**（PyMC） → **第13〜15章**（capstone / 失敗 / 運用）

- 目安：4〜6 ヶ月、週 5〜8 時間（集中して進める場合は 3〜4 ヶ月も可）

### ルート B：ML/統計は経験あり、vol-01 未読

**第1章**（最小前提）→ **第1〜3章**（vol-01 との接続を確認） → 以降はルート A と同じ

- 目安：ルート A + 1〜2 週間（第1章の理解と環境構築）

### ルート C：scikit-learn だけ先に習得したい（短期目的なら可）

第1章スキップ → 第1〜3章 → **第4〜8章のみ** → 第14章 capstone の scikit-learn 部分

- **必ず含めるべき安全条件**：**第8章（データリーク・CV の規律）は飛ばさない**。ここを飛ばした sklearn 運用は、材料研究では過学習・過信のリスクが大きい
- 想定：vol-02 の PyMC 部分は必要になった時点で再訪

### ルート D：PyMC だけ先に習得したい（短期目的なら可）

第1章スキップ → 第1〜3章 → **第9〜12章のみ** → 第14章 capstone の PyMC 部分

- **必ず含めるべき安全条件**：**第8章の「データリーク・CV の考え方」だけは読み通す**（ベイズの予測性能評価にも通用する）。加えて第5章の Skill 契約の書き方を軽く押さえる
- 想定：scikit-learn 部分は必要になった時点で再訪

---

## 2.7 vol-02 の到達点（章末に持ち帰ってほしいこと）
...
- 次章（第3章）では、**ARIM データに現れる統計的課題**——小サンプル・階層構造・反復測定・ロット差・測定誤差・物理制約——を具体的に見ていく
...
- 第3章 ARIM データに現れる統計的課題（次章）
- 第4章 Scikit-learn と PyMC の全体像・使い分け（次々章）
