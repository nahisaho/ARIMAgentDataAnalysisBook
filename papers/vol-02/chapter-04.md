# 第4章 Scikit-learn と PyMC の全体像・使い分け
...
> - AI エージェント（GitHub Copilot CLI）と Jupyter MCP を経由した scikit-learn / PyMC の呼び方の**方針**を知る（実装は第5章以降）
> - Stan は**対応表のみ**（付録B）で扱い、実行環境（cmdstanpy）は本書外である旨を確認する
>
> **本章で扱わないこと**
> - 具体的なコード・パラメータ設定・ハンズオン（第5章以降）
> - モデル選択の統計的詳細（AIC / BIC / WAIC / PSIS-LOO の使い分けは第8章・第11章）
> - Skill テンプレートの完全定義（付録A）

---

## 4.1 なぜ「2 本の柱」なのか（第2章・第3章の復習）

第2章では、vol-01 の Skill だけでは扱いきれない 4 局面（多変数・ばらつき分離・少データ予測・変数寄与）を挙げ、それに応える柱として **scikit-learn Skill** と **PyMC ベイズ Skill** を据えました。

第3章では、ARIM データに現れる 5 つの統計的課題（小サンプル・階層構造・反復測定・ロット差・物理制約）を分類しました。

**この 4 局面 × 5 課題を、なるべく少ない道具立てでカバーするための最小構成が「scikit-learn + PyMC」の 2 本**——これが本書の設計思想です。

> [!TIP]
> 「なぜ 3 本、4 本ではないのか」——道具の数が増えるほど、Skill の書き方・テスト・provenance の整合を保つコストが線形以上に増えます。本書は **「まず 2 本で足場を築き、必要になった時点で拡張する」** 方針を採ります。拡張候補は §4.7 で挙げます。

---

## 4.2 使い分けマップ

**scikit-learn を選ぶ場面** と **PyMC を選ぶ場面** を、1 枚のマップに落とします。境界は厳密ではなく、両者を組み合わせるケースも多い（第14章 capstone）ことを念頭に置いてください。
...
flowchart TD
    ...
    Q2 -- "予測・分類・PCA・PLS・回帰" --> S1["scikit-learn<br/>（第4〜8章）<br/>※小サンプルでは単純モデル<br/>+ CV 設計を重視"]
    Q2 -- "反復測定・ロット差・<br/>階層・物理制約が本質" --> Q3
    Q3 -- YES --> P1["PyMC<br/>（第9〜12章）"]
    Q3 -- NO --> S1
    S1 --> Cap["組み合わせ:<br/>特徴量エンジニアリング<br/>→ ベイズ推定"]
    P1 --> Cap
    Cap --> Result["第14章 capstone<br/>（第14章）"]
```
...
| **小サンプル（n=数十）で予測・分類・PCA/PLS**が主目的 | ◯（単純モデル+CV設計必須、第8章） | ◯（事前分布で正則化） |
...
> [!IMPORTANT]
> **n が小さくても sklearn を選ぶ場面は多い**：n=30 のスペクトルに PLS を当てる、n=50 の表形式で PCA と単純線形回帰、といった**予測・次元削減が主目的**の場面では sklearn + 強い CV 設計（第8章）が第一選択です。PyMC が優位になるのは「n が小さい」ことよりも、**不確かさ・階層構造・物理制約を明示したい**ときです。

### 「両方使う」が最も多い

実務では、**特徴量エンジニアリングは scikit-learn、モデル本体は PyMC** という組み合わせが多くなります。第14章 capstone はこの組み合わせを想定しています。

---

## 4.3 scikit-learn の位置づけ
...
- **第5章**：Skill 契約の書き方（データ分割・Pipeline・provenance のひな型）
- **第6章**：教師あり学習（線形・木・PLS）、データリーク防止の分割契約
- **第7章**：教師なし学習（PCA・クラスタリング）
- **第8章**：交差検証（CV）とデータリーク——**本書の最重要規律**
- **第9章**：モデル解釈（SHAP / PDP）
...
| 代替 | 特徴 | 本書での扱い |
|---|---|---|
| **statsmodels** | 頻度論の推論（p 値・CI）が充実 | 第8章末で「頻度論的仮説検定が必要な場面」に軽く言及 |
| **XGBoost / LightGBM** | 表形式データで scikit-learn より高性能な場合が多い | 第6章または付録で発展候補として短く紹介、Skill 化の作法は同じ |
| **PyTorch / JAX (深層学習)** | 大規模データ・画像・音声で強力 | vol-03 候補（vol-02 では扱わない） |
| **AutoML（H2O / TPOT 等）** | モデル選択を自動化 | エージェント自身がモデル選択を担うため、AutoML は本書の関心外 |

> [!NOTE]
> **本書は「モデル性能の限界を追う」本ではありません**。XGBoost で 2 ポイント精度が上がっても、データリーク・provenance の不備で 30 ポイント損することがあります。**規律 > 最高性能**——これが第4〜8章の姿勢です。
...
- **第10章**：ベイズの基本概念（事前・事後・尤度・事後予測）
- **第11章**：単純なベイズモデル（線形回帰・ロジスティック回帰のベイズ版）、章末で NumPyro / JAX の準備
- **第12章**：階層モデル（NumPyro バックエンド、合成階層データ）
- **第13章**：MCMC の実務と限界（$\hat{R}$ / ESS / divergences、Stan との対応表参照）
- **第14章 capstone**：sklearn × PyMC の連結
...
| **NumPyro（単独）** | JAX ベースで高速、確率プログラミング DSL | 第11章末以降、**PyMC のバックエンドとして**導入。単独利用は本書外 |
...
> [!IMPORTANT]
> **PyMC の学習コストを甘く見ないでください**。事前分布の設計・診断の読み方・divergences への対処——scikit-learn の「fit して predict」より学ぶことが多いです。ただし一度習得すれば、**小サンプル・階層構造・物理制約**を含む材料研究では、他の道具では出せない結論に到達できます（第10章冒頭で改めて動機を語ります）。

---

## 4.5 バックエンドの段階導入（PyMC）

第1章・第2章で触れたとおり、PyMC のバックエンドは章によって切り替えます：

| 章 | サンプラー | 追加インストール | 理由 |
|---|---|---|---|
| 第9〜10章前半 | **PyTensor 上のデフォルト NUTS** | なし | 追加依存なしで動く、エラーが Python 的 |
| 第11章末（JAX 準備、5 ページ相当） | JAX / NumPyro の導入 | `pip install numpyro jax` | 第12章以降のサンプラー切り替え準備 |
| 第12章以降 | **NumPyro NUTS**（`nuts_sampler="numpyro"`） | 上記済み | 大規模階層モデルで PyTensor 版より高速 |

> [!TIP]
> サンプラー切り替えは、モデル定義コード（`with pm.Model():` の中身）は変えず **`pm.sample(..., nuts_sampler="numpyro")`** の 1 行で行えます。PyTensor は「計算グラフ基盤」、NumPyro は「NUTS の実装差し替え」——層が違う概念であることに注意してください。第11章末に PyMC 側の設定と JAX 側の準備を集約します。

---

## 4.6 AI エージェント × Jupyter MCP からの使い方（方針）
...
| 規律 | 対応章 |
|---|---|
| **CV とデータリークの契約**：Skill が受け付ける分割方法を明示 | 第5章（契約設計） / 第6章冒頭（最低限） / 第8章（詳細 CV） |
| **MCMC の診断結果を必ずレポート**：$\hat{R}$ / ESS / divergences | 第13章 |
| **サンプラー設定を provenance に残す**：chain / draws / tune / target_accept | 第11章・付録A |
| **事後分布アーティファクトの保存**：`InferenceData` を NetCDF で保存 | 第11章・付録A |

### 注意：エージェントに「モデル選択の暴走」をさせない

第2章 §2.2 で挙げた **循環設計問題の統計版** は、この作業ループで最も起きやすい落とし穴です。**評価指標は Skill 契約に事前に書き**、エージェントに「指標の選び直し」を許可しない設計にします（第5章・第8章）。

---

## 4.7 拡張候補（本書では扱わないが、必要になる場面）
...
| **時系列モデル**（Prophet / statsforecast） | プロセス時系列の予測 | vol-02 では基本的な時系列扱いのみ（第3章 §3.4） |
...
## 4.8 第5章への橋渡し（章末チェックリスト）

以下がすべて「はい」であれば、第5章に進めます。
...
第4〜8章では、まず **scikit-learn Skill の書き方** から入ります。「fit して predict」の裏で守るべき規律——特に CV とデータリーク——が主題です。
...
- **予測精度が主目的で n が数百以上** → scikit-learn（第4〜8章）、**不確かさ・階層構造・物理制約** → PyMC（第9〜12章）、**両方** → capstone（第13〜15章）
...
- AI エージェント × Jupyter MCP の作業ループは vol-01 と同じだが、**CV・診断・provenance の 4 規律**が第5章以降で加わる
- 次章（第5章）から、**scikit-learn Skill の実装**に入る
