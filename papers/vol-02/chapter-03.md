...
flowchart TD
    ...
    Q1 -- YES --> Q2{"データ量は？"}
    Q1 -- NO --> Q3{"不確かさ・階層構造を<br/>扱いたい？"}
    Q2 -- "n >= 数百〜千" --> S1["scikit-learn<br/>（第II部・第III部）"]
    Q2 -- "n < 100" --> Q3
...
| **XGBoost / LightGBM** | 表形式データで scikit-learn より高性能な場合が多い | 第5章末で「性能が必要な場面」の候補として紹介、Skill 化の作法は同じ |
...
- ライセンス：Apache 2.0
- バックエンド：PyTensor（デフォルト）／ NumPyro（JAX ベース）／ NutPie（Rust ベース、Experimental）
...
| 章 | バックエンド | 追加インストール | 理由 |
|---|---|---|---|
| 第9〜10章前半 | **PyTensor**（デフォルト） | なし | 追加依存なしで動く、エラーが Python 的 |
| 第10章末（JAX 準備、5 ページ相当） | JAX / NumPyro の導入 | `pip install numpyro jax` | 第11章以降のバックエンド切り替え準備 |
| 第11章以降 | **NumPyro**（PyMC のバックエンド） | 上記済み | 大規模階層モデルで PyTensor より高速 |

> [!TIP]
> バックエンド切り替えは、モデル定義コードは変えず **`pm.sample(..., nuts_sampler="numpyro")`** の 1 行で行えます。第10章末に PyMC 側の設定と JAX 側の準備を集約します。
...
| 規律 | 対応章 |
|---|---|
| **CV とデータリークの契約**：Skill が受け付ける分割方法を明示 | 第7章 |
...
| **Gaussian Process 回帰**（sklearn / GPy / GPyTorch） | 連続変数の補間、小サンプル回帰 | 第9〜10章で PyMC の GP 実装を軽く紹介 |
