...
- **「NFFA-SEM の分類で 92% 出た CNN」を自分の SEM に転移したら、装置差で 60% を割った**（→ 装置固有性 × fine-tune 判断の欠落）
...
- **第11章 Foundation Model 利用時の hallucination 対策**：pretraining データとの領域ギャップ検知、attribution の sanity check[^adebayo]
...
- **付録 A の provenance 拡張**：`gpu_backend`, `cuda_version`, `cudnn_deterministic`, `random_seed_per_worker`, `weights_uri`, `weights_sha256`, `weights_license`, `pretraining_dataset_license`, `tolerance` を Skill 契約に必須化
...
> [!WARNING]
> `cudnn.deterministic=True` はパフォーマンス低下と一部演算の未対応を伴います。「bit 単位の一致」ではなく **「差分が追跡できる範囲」** を目標にする現実的な妥協が必要です（第4章で具体化）。
...
vol-03 は 2 番目の設計を採ります。**予測確率**（softmax）ではなく **calibrated predictive uncertainty**（deep ensemble variance、MC-Dropout variance、conformal prediction interval）を、停止ゲートの入力に使います（第8-9章）。
...
- **URL**：https://materialsdata.nist.gov/handle/11256/964
- **本書での位置づけ**：**少データ現場**の再現用対比（第7章 転移学習）

### データセット 4：MatBench の 1 タスク（対比、公開）

- **内容**：`matbench_perovskites` など、表形式物性予測タスクの 1 つ
- **サイズ**：数千〜1 万件
- **ライセンス**：MIT / CC-BY-4.0（`matminer` 経由、詳細は各タスクごとに公式で確認）
...
- vol-02 第2章「ARIM データに現れる統計的課題」、第7章「CV とデータリーク」、第9章「不確かさ入門：頻度論の限界と Bayesian への橋」、第11章「階層モデル：反復測定・ロット差・測定誤差」、第13章「総合ハンズオン（Advanced Capstone）：sklearn → PyMC → 階層モデル」
...
- UHCS: https://materialsdata.nist.gov/handle/11256/964
