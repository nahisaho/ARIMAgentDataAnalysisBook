...
| `pretraining_dataset_license` | `不明` を含む | 事前学習データの法的リスク |
...
- **事前学習重み仕様**：`weights_uri` / `revision`(commit hash) / `weights_sha256` / `weights_license` / `pretraining_dataset_license` / `safetensors_available`
...
- **vol-03 Layer 2 (重み)**：`pretrained_weights.weights_uri` / `pretrained_weights.revision` / `pretrained_weights.weights_sha256` / `pretrained_weights.weights_license` / `pretrained_weights.pretraining_dataset_license` / `pretrained_weights.safetensors_available`
...
pretrained_weights:
  ...
  pretraining_dataset_license: "unknown"
