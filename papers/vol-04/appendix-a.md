...
> [!IMPORTANT]
> **Fatal 名の解釈規約（N-3 対応）**：本付録の `prohibited_actions` に列挙される `Ch{n}.{name}: fatal` の各エントリは、**概念的グルーピング**（読者が「この状況ではこの fatal 群が発火する」と把握するための索引）である。実装時には、home chapter（`Ch{n}` 章）の canonical `prohibited_actions` セクションに定義された bare-form fatal 名を SoT として参照すること。付録内の名前と home chapter の canonical bare-form 名は概念的にマッピングされるが、**実装 handler の fatal 発火は home chapter の canonical 名を使用する**。例：`Ch7.claim_did_without_parallel_trends_evidence` は概念名で、実際は Ch7 §7.2 の `use_pre_trend_p_value_below_threshold_without_review` や `report_parallel_trends_pass_on_p_value_only` に対応する。cross-chapter で fatal を参照する詳細な bare-form → prefix-form マッピングは Ch14 §14.6 を参照。
...
skill:
  ...
  prohibited_actions:
    ...
    - Ch14.silently_remove_covariate_from_approved_adjustment_set: fatal  # §14.3.2 fatal (check_id: silent_confounder_removal_check)
```
...
audit_manifest_v1:
  ...
  detection_check_results:                               # Ch14 §14.4.1 canonical shape
    # causal_general (§14.1.1-§14.1.6)
    dag_misspecification_check: pass
    collider_bias_check: pass
    positivity_violation_check: pass
    unwarranted_extrapolation_check: pass
    cate_over_individualization_check: not_applicable    # ATE のみ、ITE 未評価
    refutation_gate_skip_check: pass
    # doe_general (§14.2.1-§14.2.4)
    randomization_integrity_check: not_applicable        # 観測研究
    blocking_design_integrity_check: not_applicable
    response_surface_gp_extrapolation_check: not_applicable
    taguchi_sn_ratio_interpretation_check: not_applicable
    # agentic_specific (§14.3.1-§14.3.9)
    silent_dag_modification_check: pass
    silent_confounder_removal_check: pass
    agentic_sensitivity_skip_check: pass
    silent_intervention_logging_check: pass
    seed_overwrite_check: not_applicable                 # 観測研究（RNG 不使用）
    unauthorized_broadcast_check: pass
    silent_facility_standard_inheritance_check: pass
    sequential_bayesian_prior_chain_break_check: not_applicable
    transitive_evidence_chain_invalidation_check: pass
  audit_result_summary:
    total_checks_expected: 19
    pass_count: 11
    fail_count: 0
    not_applicable_count: 8                              # applicability_manifest で pre-registered
    critical_fails: []
...
- [ ] Ch14 §14.4 canonical 19 check_id セットが完全に present（causal_general 6 + doe_general 4 + agentic_specific 9）
- [ ] `not_applicable` は applicability_manifest で事前宣言済み
- [ ] `evidence_chain_sha256` が Ch13 §13.4.4 canonical 17 fields で再計算可能
