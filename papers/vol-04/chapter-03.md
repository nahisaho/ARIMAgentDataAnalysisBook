...
この分業は、「1 つのライブラリで全部やる」設計ではなく、**「Skill ごとに最適なライブラリを 1 つ選ぶ」設計**を促します。エージェントに与える権限も、この分業に沿って設計するのが自然です。
...
|---|---|---|---|---|
| **DoWhy** | **Identification + Refutation の司令塔**。DAG から backdoor / frontdoor / IV を自動判定し、refutation ツール（placebo, random common cause, data subset）を提供 | 観測データからの ATE / ATT 推定パイプラインの骨格 | networkx, pandas, EconML（optional） | **第5・6・9章の主軸** |
| **EconML** | ML-based estimator の実装。DR-Learner / DML / Meta-Learners (S/T/X/DR/R) / Causal Forest / DeepIV 等 | CATE 推定、high-dimensional confounder | scikit-learn, PyTorch（DeepIV） | **第6・8章の主軸** |
...
1. **DoWhy + EconML の束**：DoWhy が identification と refutation を、EconML が estimator を担当する。両者は API レベルで結合しており、`dowhy.CausalModel` の `estimation_method` に EconML の Learner を差し込める
...
| ライブラリ | 主な責務 | 得意領域 | 依存関係 | vol-04 での位置 |
|---|---|---|---|---|
| **pyDOE2** | 古典 DoE の実装。Full factorial / Fractional factorial / Central Composite / Box-Behnken / Latin Hypercube / Plackett-Burman | 実験計画表の生成、randomization、blocking | numpy, scipy | **第10・11章の主軸** |
...
- **SMT の Kriging は外挿に弱い**：応答曲面の予測範囲を `counterfactual_scope_gate`（第4・8章）で制限（第11・14章）
...
**CausalPy は PyMC の "準実験特化ラッパー"** です。以下は同等性の要点：

- `causalpy.pymc_experiments.DifferenceInDifferences` は、内部で PyMC モデル（`pm.Normal` の切片・傾き・交互作用項）を組み立てる
- 読者が独自の事前分布を差し込みたい場合、CausalPy の `formula` API か、PyMC で直接書き下すかの二択
...
| SMT（外挿範囲の判定） | 準自律 | **`counterfactual_scope_gate`** | 外挿は Ch8 の閾値実装で自動判定＋逸脱時 Human review |
...
自分の研究テーマに対して、最初に手を出すライブラリを 1 つ選ぶための判断フローです。
...
flowchart TD
  ...
  Q4 -->|DiD / IV / RDD /<br/>Synthetic Control| Q5{Frequentist or<br/>Bayesian?}
  Q5 -->|Frequentist| L2["<b>linearmodels</b><br/>第7章"]
  Q5 -->|Bayesian| L3["<b>CausalPy → PyMC</b><br/>第7,9章"]
  Q3 -->|実験データ| L4["<b>統計 ML の再利用</b>+<br/>DoWhy で refutation<br/>第6,9章"]
  Q2 --> Q6{探索段階?}
...
2. **データ由来と識別戦略で分岐**：観測 vs 実験、backdoor vs 準実験
3. **Frequentist / Bayesian の哲学**：既存資産（vol-02 で PyMC 経験あり）と組織方針で選ぶ
4. **単一 Skill = 単一ライブラリ**：複数ライブラリを 1 つの Skill に混ぜない（第4章の Skill 単位性）
...
skill:
  ...
  authorization_gates:
    dag_authorization:
      required_for: [causal_graph_uri change, identification_strategy change]
      approver: research_lead
    variable_selection_authorization:
      required_for: [confounders_declared change]
      approver: research_lead
    intervention_execution_authorization:
      required_for: physical experiment execution
...
- **library_stack**：識別・推定・検算にそれぞれどのライブラリを使うか（**混在を許すが Skill 契約に明記**）
...
- **refutation_tests_required**：DoWhy の refutation ツール名。Skill は refutation pass しない限り結果を返さない（第9章）
- **authorization_gates**：3 層承認 + `counterfactual_scope_gate`

**第4章で完全展開する項目**：`experimental_design_provenance`（DoE 章の Skill 契約）、`randomization_seed`、`blocking_factors`、`sequential_experiment_stop_condition`（**vol-05 へ委譲**）。
...
- [ ] **§3.6 のフローチャート**を辿って、**自分の研究テーマに対する初手ライブラリを 1 つ**選んだ
...
- [ ] **SMT の外挿 = `counterfactual_scope_gate`** で制御することを理解した（第8・11章で operational 実装）
