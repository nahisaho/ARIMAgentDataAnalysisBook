...
> - **エージェントが DAG を勝手に書き換える 4 つの典型パターン**と、それぞれの `dag_authorization` ゲートでの防ぎ方を言える
> - 本書で使う **合成 causal 演習データ**（`data/synthetic-causal/`）の構造を理解し、自分の環境に取り込める
> - 章末演習で **「自分のデータ」の 3 変数 DAG を描き、Type A〜E に分類**した状態で第3章に進める
>
> **本章で扱わないこと**
> - DAG 記法の形式的定義（第5章）
...
| データ型 | よくある treatment | よくある outcome | 典型 confounder | 典型 mediator | 典型 collider（避けるべき adjustment 対象） |
|---|---|---|---|---|---|
| **スペクトル型** | 前処理温度、雰囲気ガス | ピーク強度、半値幅 | 装置キャリブレーション、標準試料の由来、測定日 | 前駆体の分解生成物、粒径 | 「後段の別工程で得た品質判定」——treatment と outcome の両方が原因 |
| **クロマトグラム・時系列型** | 反応条件、触媒配合 | 生成物分布、収率 | バッチ差、季節（湿度）、担当オペレータ | 中間生成物濃度、反応温度プロファイル | 「精製後のスペクトル純度検査結果」——精製工程が両者に依存 |
| **画像・顕微鏡型** | 熱処理条件、成膜圧力 | 粒径分布、欠陥密度 | 装置解像度、電子線ドリフト、試料ドリフト、撮影時刻 | 表面粗さ、結晶配向 | 「他手法（TEM/AFM）で得た同じ試料の別判定」——両者と共通の観測選択に依存 |
| **回折・散乱パターン型** | 合成温度、圧力、時間 | 格子定数、結晶相 | 装置角度精度、標準試料 offset、beam alignment | 格子歪み、応力 | 「Rietveld 精密化の残差」——treatment とセットで計算 |
| **表形式・プロセス条件型** | 全プロセス変数の任意 | 全物性の任意 | オペレータ、ロット、季節、装置世代 | 中間物性値、副生成物量 | 「後段の品質分類ラベル」——treatment と outcome の両者に依存 |
| **マルチモーダル統合型** | 統合プロセス全体 | 統合物性 | 上記の各モダリティ固有 confounder が **同時に効く** | 各モダリティの中間表現 | 「モダリティ間で共通する downstream 判定」 |
...
- **collider**：treatment と outcome の **共通の結果** となる変数。DAG では $T \rightarrow K \leftarrow Y$。**collider を adjustment に含めると spurious な相関が生じる**（第5章 M-bias / butterfly bias）
...
**Case D：装置差は mediator**

- 研究目的：「新しい標準操作手順（SOP）の効果」を推定したい
- 新 SOP は装置設定を変える → 装置設定が outcome に効く
- DAG：$T \rightarrow D \rightarrow Y$（$D$ は $T \rightarrow Y$ の経路）
- 対処：**装置差を adjustment に含めると "直接効果" のみが残り、SOP の総合効果が歪む**——媒介分析（第5章）で indirect / direct 効果を分解
...
**positivity（正値性、common support）** とは、「adjustment set の各値において、treatment が両群（介入群と対照群）に一定確率で観察される」という仮定です。$P(T = 1 \mid X = x) \in (0, 1) \; \forall x$。
...
- **状況**：Skill 実行中、エージェントが outcome model の R² を上げるために、`後段の別工程で得た品質判定` を共変量に追加
- **問題**：この変数は treatment と outcome の共通結果（collider）——**adjustment に入れると M-bias / butterfly bias が発生**
...
- **状況**：causal-learn の PC アルゴリズムが提案した DAG を、エージェントが `causal_graph_uri` を上書きして採用
...
- **防ぎ方**：`identification_strategy` を provenance に pin し、**変更時は Skill を再認証**（新しい Skill バージョンとして扱う）。エージェントは Skill を切り替えられるが、Skill 内部で戦略を変えることは禁止

### 4 パターンと `dag_authorization` / `variable_selection_authorization` の対応

| パターン | 主に迂回されるゲート | 検知フィールド |
|---|---|---|
| 1. Collider を追加 | `variable_selection_authorization` | `confounders_declared` との差分 |
| 2. 探索 DAG を無承認採用 | `dag_authorization` | `causal_graph_sha256` の変化 |
| 3. Mediator を格上げ | `variable_selection_authorization` | `estimand_type` との整合性 |
| 4. Identification 戦略の silent 切替 | Skill 版数管理（第4章） | `identification_strategy` の変化 |
...
└── generation_scripts/          # 全データの生成スクリプト（再現用）
    ├── generate_tabular.py
...
| **C：IV** | 装置キャリブレーション日を instrument に | 第7章 |
| **D：Mediator（frontdoor 条件を満たす例と満たさない例）** | 媒介分析での direct / indirect 効果の分解 | 第5-6章 |
| **E：Collider** | Collider を adjustment に入れた場合の bias（M-bias） | 第5章 |
...
```bash
git clone https://github.com/example/synthetic-causal-arim data/synthetic-causal
...
`docs/exercises/ch02_my_causal_map.md` に保存してください。第13a章 Capstone で、この最初の紙とペンの DAG が、そのプロジェクトの成果物に化けます。**vol-01〜03 では紙メモは「あってもなくてもよい」でしたが、vol-04 では紙 DAG は成果物の一部**——`causal_graph_uri` の起点になります。
