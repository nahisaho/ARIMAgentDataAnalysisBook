...
|---|---|---|---|---|
| 1 | **identification validity** | 与えられた DAG と変数役割で、目標 estimand が **identifiable** である | `identification_strategy` + `causal_graph_sha256` + **戦略別 identifiability check**（backdoor/frontdoor は DoWhy、IV/DiD/RDD/SC は CausalPy/linearmodels 側の review artifact、pgmpy 直接推論は自作レポート）+ `identification_report_uri` | 第5-7章 |
...
| メトリック | 境界の向き | 初期値の決め方 |
|---|---|---|
| `distance_threshold` | 上限 | 学習データ内部で計算した距離分布の **95 パーセンタイル** |
| `cate_variance_threshold` | 上限 | 学習データ内 CV での CATE 予測分散の **95 パーセンタイル** |
| `neighbor_density_threshold` | **下限** | 学習データ内部での近傍密度分布の **5 パーセンタイル**（または実効サポートの下限） |

第8-9章で domain-specific な再校正手順を示します。
...
> [!IMPORTANT]
> **利益相反への対処**：`research_lead` や `PI` が **同時にデータ生成者・因果的主張の受益者**である場合、自己 review になり承認の意味が失われます。`approver_independence` フィールドで、**利益相反時は独立レビュアー（例：ARIM 施設の causal review board）を経由する**ことを契約に明示します。ARIM のような共用施設ではこの governance gap への対処が必須です。
...
|---|---|---|---|---|---|
| 1 | Silent identification strategy switch（Skill 内部で backdoor→IV 等） | **fatal** | `identification_strategy` の差分 | `dag_authorization` | パターン 4 |
| 2 | Unauthorized DAG modification | **fatal** | `causal_graph_sha256` の差分 | `dag_authorization` | パターン 2 |
| 3 | Adjustment of collider without authorization | **fatal** | `confounders_declared` と DAG の整合、`colliders_declared` 越境 | `variable_selection_authorization` | パターン 1 |
| 4 | Mediator を confounder に格上げ | **fatal** | `mediators_declared` / `confounders_declared` / `estimand_type` の差分 + DAG diff | `dag_authorization` + `variable_selection_authorization` | パターン 3 |
...
| 12a | Scope gate 境界近傍での反実仮想（距離・分散が閾値付近） | warning（`low_confidence` 出力） | `counterfactual_scope_gate` 判定（境界域） | `counterfactual_scope_gate` | — |
| 12b | Scope gate 逸脱・SMT 応答曲面の明示的外挿 | **fatal**（fail-close、結果を返さない） | `counterfactual_scope_gate` 判定（fail） | `counterfactual_scope_gate` | — |
...
skill:
  ...
  causal_question_type: type_b_cate  # 第1章の 5 Type のいずれか
  ...
  counterfactual_scope_gate:
    ...
    distance_threshold: 3.0            # 上限：学習データ距離の 95 パーセンタイル
    neighbor_density_metric: k_nn_density
    neighbor_density_threshold: 0.1    # 下限：学習データ密度の 5 パーセンタイル
    cate_variance_metric: prediction_variance
    cate_variance_threshold: 0.25      # 上限：CV 予測分散の 95 パーセンタイル
  ...
  prohibited_actions:
    ...
    - scope_gate_boundary: warning              # low_confidence 出力
...
- [ ] **§4.8 Table 4.4 禁止事項（12a/12b を含む 13 項目）**を、Severity（fatal / warning）付きで整理できる
